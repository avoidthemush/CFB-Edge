"""
Second-split confirmation for the narrow-odds-range EV result. Original
(train 2021-2023, validate 2024) showed an erratic, non-monotonic
pattern across EV thresholds (+5.4% -> +2.5% -> -0.3% -> -0.2% -> +3.3%)
- a real warning sign of noise, not signal. Retrains on train 2021-2022,
validates on 2023 to check whether this holds independently.
"""
import pandas as pd
...
def run():
    ...
    train_df = full_df[(full_df["season"] >= 2021) & (full_df["season"] <= 2022)]
    ...
    print(f"Train: {len(X_train)} games (2021-2022) | Validate: {len(X_val)} games (2023)\n")
    ...
    print("=== Results by EV threshold, narrow odds range, RECHECK (2023) ===")
