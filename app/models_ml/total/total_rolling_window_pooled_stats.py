"""
Computes exact pooled significance + bootstrap for the rolling 1-year
training window approach, using all 4 folds already run (including the
2025 fold already spent in total_rolling_window_confirm.py). Not a new
spend - rigorously quantifying a result already revealed, not peeking
at new information.
"""
import numpy as np
import pandas as pd
...
BREAKEVEN = 0.524
ROLLING_FOLDS = [(2021, 2022), (2022, 2023), (2023, 2024), (2024, 2025)]
N_BOOTSTRAP = 10000
...
def run():
    ...
    for train_year, test_year in ROLLING_FOLDS:
        ...
        for threshold in GAP_THRESHOLDS:
            ...
            results_by_threshold[threshold]["wins"] += int(correct.sum())
            results_by_threshold[threshold]["total"] += len(correct)
            results_by_threshold[threshold]["correct_arr"].extend(correct.astype(int).tolist())

    print("POOLED RESULTS + BOOTSTRAP (rolling 1-year training, all 4 folds)\n")
    ...
    for threshold in GAP_THRESHOLDS:
        ...
        print(f"Gap >= {threshold}: {wins}/{total} = {pooled_rate:.1f}% | p={pvalue:.4f}")
