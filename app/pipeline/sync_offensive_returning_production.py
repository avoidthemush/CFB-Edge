...
from app.models import OffensiveReturningProduction, Team
...
def sync_returning_production_for_year(year: int, tracker: ApiUsageTracker):
    ...
    later. Defense side lives separately - see
    calc_defensive_returning_production.py and DESIGN_DECISIONS.md.
    """
    ...
    try:
        with cfbd.ApiClient(configuration) as api_client:
            ...
            for r in results:
                ...
                existing = db.query(OffensiveReturningProduction).filter(
                    OffensiveReturningProduction.team_id == team_id,
                    OffensiveReturningProduction.year == year,
                ).first()
                ...
                if existing:
                    ...
                    db.add(OffensiveReturningProduction(team_id=team_id, year=year, **fields))
                    inserted += 1
...
def backfill_returning_production(start_year: int = HISTORICAL_START_YEAR, end_year: int = CURRENT_SEASON):
    ...
    print(f"Backfilling offensive returning production from {start_year} to {end_year}...")
