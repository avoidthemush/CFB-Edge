"""
Fixes the core mistake: betting on DISAGREEMENT with the market (edge
gap) isn't the same as betting on POSITIVE EXPECTED VALUE. A well-
calibrated model (confirmed via calibration check - genuinely accurate)
can still lose money if disagreement-based betting ignores the actual
PRICE being offered. This computes real EV per bet: our probability x
potential payout, minus (1 - our probability) x stake.
"""
import pandas as pd
...
def expected_value(our_prob, odds):
    """EV per $1 staked: our_prob * payout - (1 - our_prob) * 1."""
...
def run():
    ...
    print("=== Results by EV threshold (bet whichever side has EV >= threshold) ===")
    for threshold in EV_THRESHOLDS:
        bet_home = df_val[df_val["ev_home"] >= threshold].copy()
        ...
        bet_away = df_val[df_val["ev_away"] >= threshold].copy()
        ...
        print(f"  EV>={threshold}: {len(all_bets)} bets, {win_rate:.1f}% win, "
