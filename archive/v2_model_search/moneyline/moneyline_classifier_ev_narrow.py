"""
Narrower odds-range test - MIN_ODDS/MAX_ODDS tightened from (-300, 300)
to (-200, 150) to see if excluding moderate longshots (not just extreme
ones) closes the remaining small gap (-1.1% to -3.5% ROI at the wider
cap). If still negative here, that's a real signal this feature set/
model lacks precision anywhere on the board, not just at the tails.
"""
import pandas as pd
...
def run():
    ...
    print("=== Results by EV threshold, narrow odds range ===")
