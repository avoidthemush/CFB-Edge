"""
Directly tests whether the model's stated CONFIDENCE means anything for
weeks 1-4 - i.e., when it says '60% sure,' is it actually right ~60% of
the time? A model can output a confident-looking number without that
number being trustworthy. Compares early season (1-4) calibration
against mid/late season (5+) as a baseline for what "working" looks like.
"""
import pandas as pd
...
def run():
    ...
    for label, week_filter in [
        ("WEEKS 1-4 (early season)", preds["week"] <= 4),
        ("WEEKS 5+ (mid/late season, our current working rule)", preds["week"] > 4),
    ]:
        subset = preds[week_filter]
        print(f"\n=== {label} - calibration check ===")
        print(f"{'Stated confidence':<20}{'# predictions':>15}{'Actual accuracy':>18}{'Well calibrated?':>20}")
        for low, high in CALIBRATION_BUCKETS:
            bucket = subset[(subset["confidence"] >= low) & (subset["confidence"] < high)]
            if len(bucket) == 0:
                print(f"{low:.2f}-{high:.2f}{'':>15}no predictions")
                continue
            actual_acc = bucket["correct"].mean() * 100
            stated_midpoint = (low + high) / 2 * 100 if high < 1.01 else 85
            gap = actual_acc - stated_midpoint
            calibrated = "reasonable" if abs(gap) < 10 else "MISCALIBRATED"
            print(f"{low:.2f}-{high:.2f}{'':>7}{len(bucket):>15}{actual_acc:>17.1f}%{calibrated:>20}")
