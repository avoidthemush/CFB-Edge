"""
Hyperparameter comparison scoped for the smaller FBS-only dataset
(2,362 training rows) - the original settings (300 trees, depth 4) were
implicitly tuned for 4x more data. Tests several simpler/more
regularized configs against the 2024 internal validation split, 2025
still sealed. Reports close-game MAE (the metric that matters most),
blowout MAE, and the train/validation gap (overfitting check) for each.
"""
import pandas as pd
...
def run():
    ...
    print("=== Summary, sorted by close-game MAE (the metric that matters most) ===")
